...
public class CheckController {
	 ...
	 * 
	 * if (errors!=null) { for (FieldError fieldError : errors) {
	 * System.out.println(fieldError.getDefaultMessage()); } }else {
	 * //当不为空的时候开始发送邮件 System.out.println(user); //将前台获取到的邮箱传回到发送邮件中去 new
	 * Thread(new EmailUtil(user.getEmail(), "222", user.getUser_acc(),
	 * user.getUser_pwd())).start(); System.out.println("测试！"); }
	 * 
	 * 
	 * }
	 */
